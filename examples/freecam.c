...
void GraphicsSetup() {
  ...
  tsekM_perspective(perspective_matrix, 45.0, 16.0f/9, 0.1f, 100.0f);
...
void GetMovement() {
  ...
  if (keymap[TSEK_W]) {
    tsekM_add(camera_position, camera_position, forward, 3);
  } if (keymap[TSEK_S]) {
    tsekM_sub(camera_position, camera_position, forward, 3);
  } if (keymap[TSEK_A]) {
    tsekM_add(camera_position, camera_position, right, 3);
  } if (keymap[TSEK_D]) {
    tsekM_sub(camera_position, camera_position, right, 3);
  } if (keymap[TSEK_SPACE]) {
    tsekM_add(camera_position, camera_position, up, 3);
  } if (keymap[TSEK_LEFTSHIFT]) {
    tsekM_sub(camera_position, camera_position, up, 3);
  }
}

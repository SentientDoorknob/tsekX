...
uint32_t Gget_size_glenum(GLenum type) {
  switch (type) {
    case 0x1406: return sizeof(float);
    case 0x1404: return sizeof(int);
    case 0x8B56: return sizeof(GLboolean);
...
void tsekG_describe_buffer(tsekBuffer* buffer, tsekFormat format) {
  ...
  glad_glBindBuffer(0x8892, buffer->VBO);
  ...
  for (int i = 0; i < format.count; i++) {
    ...
    if (attribute.type == 0x1404 || attribute.type == 0x1405) {
    ...
    glad_glEnableVertexAttribArray(attribute.location);
...
void tsekG_fill_buffer(tsekBuffer* buffer, void* vertices, uint32_t vertices_size, uint32_t* indices, uint32_t indices_size) {
  ...
  glad_glBindBuffer(0x8892, buffer->VBO);
  glad_glBufferData(0x8892, vertices_size, vertices, 0x88E4);

  glad_glBindBuffer(0x8893, buffer->EBO);
  glad_glBufferData(0x8893, indices_size, indices, 0x88E4);
...
void tsekG_render_buffer(tsekBuffer* buffer, tsekShader* shader, GLenum primitive) {
  ...
  glad_glUseProgram(shader->program);

  GLenum err = glad_glGetError();

  glad_glDrawElements(primitive, buffer->index_count, 0x1405, 0);
}
...
uint32_t Gcompile_shader(GLenum type, const char* src) {
  ...
  glad_glShaderSource(shader, 1, &src, 
# 177 "src/tsekG.c" 3 4
                                 ((void *)0)
# 177 "src/tsekG.c"
                                     );
  ...
  if (!success) {
    char log[1024];
    glad_glGetShaderInfoLog(shader, sizeof(log), 
# 185 "src/tsekG.c" 3 4
                                           ((void *)0)
# 185 "src/tsekG.c"
                                               , log);
...
void tsekG_compile_shader(tsekShader* shader) {
  uint32_t vertex = Gcompile_shader(0x8B31, shader->vertex_src);
  uint32_t fragment = Gcompile_shader(0x8B30, shader->fragment_src);

  shader->program = glad_glCreateProgram();

  glad_glAttachShader(shader->program, vertex);
  glad_glAttachShader(shader->program, fragment);

  glad_glLinkProgram(shader->program);
  GLint success;
  glad_glGetProgramiv(shader->program, 0x8B82, &success);
  if (!success)
  {
    char infoLog[1024];
    glad_glGetProgramInfoLog(shader->program, 1024, 
# 207 "src/tsekG.c" 3 4
                                              ((void *)0)
# 207 "src/tsekG.c"
                                                  , infoLog);
...
void tsekG_set_uniform_handle(tsekShader* shader, tsekUniform* handle, void* data) {
  ...
  if (handle->is_matrix) {
    ...
    switch (handle->count) {
      case 4: tsekM_transpose(transposed, data, 2); glad_glUniformMatrix2fv(loc, 1, 0, (float*)transposed); break;
      case 9: tsekM_transpose(transposed, data, 3); glad_glUniformMatrix3fv(loc, 1, 0, (float*)transposed); break;
      case 16: tsekM_transpose(transposed, data, 4); glad_glUniformMatrix4fv(loc, 1, 0, (float*)transposed); break;
      default: fprintf(
# 235 "src/tsekG.c" 3 4
                      stderr
# 235 "src/tsekG.c"
                            , "Invalid Matrix Count: %d\n", handle->count);
    }
    return;
  }

  if (handle->type == 0x1406) {
    switch (handle->count) {
      case 1: glad_glUniform1fv(loc, 1, (float*)data); break;
      case 2: glad_glUniform2fv(loc, 1, (float*)data); break;
      case 3: glad_glUniform3fv(loc, 1, (float*)data); break;
      case 4: glad_glUniform4fv(loc, 1, (float*)data); break;
      default: fprintf(
# 246 "src/tsekG.c" 3 4
                      stderr
# 246 "src/tsekG.c"
                            , "Invalid Float Vector Count: %d\n", handle->count);
    }
    return;
  }

  if (handle->type == 0x1404 || handle->type == 0x1405) {
    switch (handle->count) {
      case 1: glad_glUniform1iv(loc, 1, (int*)data); break;
      case 2: glad_glUniform2iv(loc, 1, (int*)data); break;
      case 3: glad_glUniform3iv(loc, 1, (int*)data); break;
      case 4: glad_glUniform4iv(loc, 1, (int*)data); break;
      default: fprintf(
# 257 "src/tsekG.c" 3 4
                      stderr
# 257 "src/tsekG.c"
                            , "Invalid Integer Vector Count: %d\n", handle->count);
    }
    return;
  }
...
tsekUniform* tsekG_set_uniform(tsekShader* shader, const char* name, GLenum type, GLint count, int is_matrix, void* data) {
  ...
  GLint loc = glad_glGetUniformLocation(shader->program, name);
  if (loc == -1) {
    fprintf(
# 301 "src/tsekG.c" 3 4
           stderr
# 301 "src/tsekG.c"
  ...
  if (shader->uniform_cache.count >= 32) {
    fprintf(
# 306 "src/tsekG.c" 3 4
           stderr
# 306 "src/tsekG.c"
                 , "Warning: Uniform cache capacity of %d filled. Not caching uniform '%s'", 32, name);
  } else {
...
void tsekG_create_texture(tsekTexture *texture, const char *bitmap, uint32_t unit, int wrapS, int wrapT, int filterMin, int filterMax) {
  ...
  glad_glGenTextures(1, &texture->texture);

  uint32_t format = nr_channels == 3 ? 0x1907 : 0x1908;
  glad_glBindTexture(0x0DE1, texture->texture);

  glad_glTexParameteri(0x0DE1, 0x2802, texture->wrapS);
  glad_glTexParameteri(0x0DE1, 0x2803, texture->wrapT);

  glad_glTexParameteri(0x0DE1, 0x2801, texture->filterMin);
  glad_glTexParameteri(0x0DE1, 0x2800, texture->filterMax);

  glad_glPixelStorei(0x0CF5, 1);
  glad_glTexImage2D(0x0DE1, 0, format, width, height, 0, format, 0x1401, raw_texture);
...
void tsekG_bind_texture(tsekTexture* texture, tsekShader* shader, char* name) {
  glad_glActiveTexture(0x84C0 + texture->unit);
  glad_glBindTexture(0x0DE1, texture->texture);

  tsekG_set_uniform(shader, name, 0x1404, 1, 0, &texture->unit);
}

void tsekG_set_border_color(tsekTexture* texture, float* color) {
  glad_glBindTexture(0x0DE1, texture->texture);
  glad_glTexParameterfv(0x0DE1, 0x1004, color);
}

void tsekG_create_framebuffer(tsekGFramebuffer* buffer) {
  glad_glGenFramebuffers(1, &buffer->fbo);
}
...
void tsekG_create_framebuffer_attachment(tsekGFramebuffer* buffer, tsekGAttachmentType type, int width, int height, int wrapS, int wrapT, int filterMin, int filterMax) {
  ...
  glad_glGenTextures(1, &t.texture);
  glad_glBindTexture(0x0DE1, t.texture);
  glad_glBindFramebuffer(0x8D40, buffer->fbo);

  if (type == TSEKG_COLOR) {
    t.channels = 4;
    glad_glTexImage2D(0x0DE1, 0, 0x1908, width, height, 0, 0x1908, 0x1401, 
# 448 "src/tsekG.c" 3 4
                                                                                        ((void *)0)
# 448 "src/tsekG.c"
                                                                                            );
    glad_glFramebufferTexture2D(0x8D40, 0x8CE0 + buffer->color_count, 0x0DE1, t.texture, 0);
    memcpy(buffer->color + buffer->color_count, &t, sizeof(tsekTexture));
    buffer->color_count++;
  }
  else if (type == TSEKG_DEPTH) {
    t.channels = 3;
    glad_glTexImage2D(0x0DE1, 0, 0x1902, width, height, 0, 0x1902, 0x1401, 
# 455 "src/tsekG.c" 3 4
                                                                                                              ((void *)0)
# 455 "src/tsekG.c"
                                                                                                                  );
    glad_glFramebufferTexture2D(0x8D40, 0x8D00, 0x0DE1, t.texture, 0);
    memcpy(&buffer->depth, &t, sizeof(tsekTexture));
  }
  else if (type == TSEKG_STENCIL) {
    t.channels = 1;
    glad_glTexImage2D(0x0DE1, 0, 0x1901, width, height, 0, 0x1901, 0x1401, 
# 461 "src/tsekG.c" 3 4
                                                                                                          ((void *)0)
# 461 "src/tsekG.c"
                                                                                                              );
    ...
    t.channels = 4;
    glad_glTexImage2D(0x0DE1, 0, 0x88F0, width, height, 0, 0x88F0, 0x84FA, 
# 467 "src/tsekG.c" 3 4
                                                                                                                    ((void *)0)
# 467 "src/tsekG.c"
                                                                                                                        );
    glad_glFramebufferTexture2D(0x8D40, 0x821A, 0x0DE1, t.texture, 0);
    memcpy(&buffer->depth, &t, sizeof(tsekTexture));
    memset(&buffer->stencil, 0, sizeof(tsekTexture));
  }

  glad_glTexParameteri(0x0DE1, 0x2803, wrapT);
  glad_glTexParameteri(0x0DE1, 0x2802, wrapS);
  glad_glTexParameteri(0x0DE1, 0x2801, filterMin);
  glad_glTexParameteri(0x0DE1, 0x2800, filterMax);
}
...
void tsekG_destroy_framebuffer(tsekGFramebuffer* buffer) {
  glad_glDeleteFramebuffers(1, &buffer->fbo);
}
